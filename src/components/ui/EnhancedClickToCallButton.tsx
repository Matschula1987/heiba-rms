...
interface EnhancedClickToCallButtonProps {
  phoneNumber: string;
  displayName?: string;
  userId: string;
  // Entity-Referenzen
  ...
  jobId?: string;
  talentPoolId?: string;
  requirementId?: string;  // Neu: ID der Kundenanforderung
  customerId?: string;     // Neu: ID des Kunden/Interessenten
  
  createAssignment?: boolean;
  variant?: 'default' | 'outline' | 'secondary' | 'ghost' | 'link' | 'destructive';
  size?: 'default' | 'sm' | 'lg' | 'icon';
  className?: string;
  iconOnly?: boolean;      // Neu: Nur das Icon anzeigen
}

export function EnhancedClickToCallButton({
  phoneNumber,
  displayName,
  userId,
  candidateId,
  ...
  className,
  iconOnly = false
}: EnhancedClickToCallButtonProps) {
    ...
    try {
      const response = await fetch('/api/phone-integration/click-to-call', {
        ...
        body: JSON.stringify({
          ...
          jobId,
          talentPoolId,
          requirementId,    // Neue Parameter
          customerId,       // Neue Parameter
          createAssignment
        })
      });
  ...
  const showText = !iconOnly && size !== 'icon';
  ...
  return (
    <Button
      ...
      disabled={isLoading}
      className={className}
      title={`Anrufen: ${formattedPhone} (${displayName || 'Unbekannt'})`}
    >
      <Phone className={`h-4 w-4 ${showText ? 'mr-2' : ''}`} />
      {showText && formattedPhone}
    </Button>
  );
}
